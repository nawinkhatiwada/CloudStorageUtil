...
fun amazonUploadSingle(
    awsMetaInfo: AwsMetaInfo,
    uploadPath: String,
    maxFileSize: Long
): Single<TransferUpdate?>? {
...
fun amazonUploadSingle(awsMetaInfo: AwsMetaInfo): Single<TransferUpdate?>? {
    return amazonUploadSingle(
        awsMetaInfo,
        awsMetaInfo.awsFolderPath,
        TransferObservable.FILE_SIZE_UNLIMITED
    )
}
...
fun fetchAwsImage(
    serviceConfig: AwsMetaInfo.AWSConfig,
    queryPath: String
): Single<List<AWSStorageResponse>> {
...
fun getImageFromS3Buckets(
    serviceConfig: AwsMetaInfo.AWSConfig,
    queryPath: String
): List<AWSStorageResponse> {
